...
@Tag(name = UtilName.RECEIPT + UtilName.CONTROLLER)
public interface ReceiptControllerDocs {
    ...
    @PostMapping
    void create(@RequestBody ReceiptDTO dto);
    ...
    @ResponseStatus(HttpStatus.OK)
    @GetMapping("/{id}")
    ReceiptDTO getById(@PathVariable("id") String id);
    ...
    @GetMapping
    List<ReceiptDTO> getAll(@RequestParam(required = false, defaultValue = "false") Boolean deleted);
